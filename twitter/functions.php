	...
	function displayTweets($type){
		...
		if ($type == 'public'){
			...
			while($row = mysqli_fetch_assoc($result)){
				if ($whereClause == "") $whereClause = "WHERE userid = ".$row['following'];
				else $whereClause .= " OR userid = ".$row['following'];
			}
		...
		if (mysqli_num_rows($result) == 0){
			...
			while ($row = mysqli_fetch_assoc($result)){
				...
				$isFollowingQuery = "SELECT * FROM isFollowing WHERE follower = ".mysqli_real_escape_string($link, $_SESSION['id'])." AND following = ".mysqli_real_escape_string($link, $row['userid'])." LIMIT 1";
	...
	function displayTweetBox() {
		...
		if ($_SESSION['id'] > 0){
			...
			<div class="form">
			  ...
			  <button style="margin-top: 10px;" id="postTweetButton" class="btn btn-primary">Post Tweet</button>
			</div>';
		}
	}
